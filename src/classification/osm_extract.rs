...
//! Plan-Notizen), sondern nach Tag-*Werten*: [`flags_for_tag`] ist der
//! geschlossene Satz, den `rules.rs` tatsächlich abfragt. Nur Ways, für die
//! das Filter mindestens ein Flag liefert, werden behalten — und von denen
//! auch nur die Knotenkoordinaten, die ihre Ringe tatsächlich brauchen.
...
use crate::classification::rules::flags_for_tags;
...
pub fn extract_features(path: &Path) -> Result<Vec<OsmFeature>, ClassificationError> {
    ...
    reader.for_each(|element| {
        ...
        let flags = flags_for_tags(way.tags());
        if flags == 0 {
            return;
        }
        ...
        candidates.push(WayCandidate { flags, refs });
    })?;
...
fn build_geometry(coords: Vec<Coord<f64>>) -> Geometry<f64> {
...
mod tests {
    ...
    fn geschlossene_kette_wird_zu_polygon() {
        ...
        assert!(matches!(build_geometry(coords), Geometry::Polygon(_)));
    }

    #[test]
    fn offene_kette_wird_zu_linestring() {
        let coords = vec![Coord { x: 0.0, y: 0.0 }, Coord { x: 1.0, y: 1.0 }];
        assert!(matches!(build_geometry(coords), Geometry::LineString(_)));
    }

    #[test]
    fn einzelner_punkt_wird_zu_point() {
        let coords = vec![Coord { x: 0.0, y: 0.0 }];
        assert!(matches!(build_geometry(coords), Geometry::Point(_)));
    }
